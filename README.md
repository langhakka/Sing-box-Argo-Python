...
登录页面后选择：Web App 分类，，接着选择 Flask 类型，这个就是python架构，，接着源代码上传可以暂时留空，继续Continue，，接着选free计划，开启容器，上传本项目文件，main二进制文件较大，请耐心等待，运行容器
...
### 方案 A：上传项目文件部署（推荐）
1. 将打包好的 **`main.py`**、**`main`**（可执行二进制）和 **`requirements.txt`** 上传至容器，时间较长，请耐心等待
