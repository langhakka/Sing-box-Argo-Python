...
func initEnvs() {
	...
	autoAccess = strings.ToLower(getEnv("AUTO_ACCESS", "false")) == "true"
	filePath = getEnv("FILE_PATH", ".cache")
	absPath, err := filepath.Abs(filePath)
	if err == nil {
		filePath = absPath
	}
	subPath = strings.Trim(getEnv("SUB_PATH", "sub"), "/")
	...
	disableArgo = strings.ToLower(getEnv("DISABLE_ARGO", "false")) == "true"
...
func setupArgo() {
	if disableArgo || argoAuth == "" || argoDomain == "" {
		return
	}
...
func uploadNodes() {
	...
	if uploadURL != "" && projectURL != "" {
		subURL := fmt.Sprintf("%s/%s", strings.TrimRight(projectURL, "/"), subPath)
		payload, _ := json.Marshal(map[string]interface{}{"subscription": []string{subURL}})
		req, _ := http.NewRequest("POST", fmt.Sprintf("%s/api/add-subscriptions", strings.TrimRight(uploadURL, "/")), bytes.NewBuffer(payload))
		req.Header.Set("Content-Type", "application/json")
		client.Do(req)
	} else if uploadURL != "" {
		if data, err := os.ReadFile(listFilePath); err == nil {
			...
			if len(nodes) > 0 {
				...
				req, _ := http.NewRequest("POST", fmt.Sprintf("%s/api/add-nodes", strings.TrimRight(uploadURL, "/")), bytes.NewBuffer(payload))
				req.Header.Set("Content-Type", "application/json")
				client.Do(req)
			}
		}
...
func sendTelegram() {
	...
	msgData, err := os.ReadFile(subFilePath)
	if err != nil {
		return
	}
	...
	params := fmt.Sprintf("chat_id=%s&text=*%s Node Push Notification*\n`%s`&parse_mode=MarkdownV2", chatID, displayName, string(msgData))
	client := &http.Client{Timeout: 10 * time.Second}
	client.Post(tgURL+"?"+params, "application/x-www-form-urlencoded", nil)
}
...
func addVisitTask() {
	...
	client := &http.Client{Timeout: 10 * time.Second}
	req, _ := http.NewRequest("POST", "https://keep.gvrander.eu.org/add-url", bytes.NewBuffer(payload))
	req.Header.Set("Content-Type", "application/json")
	client.Do(req)
}
...
func startServices() {
	...
	fmt.Println("Starting minimal Argo tunnel service (Go native)...")
	cleanupOldFiles()
	createDirectory()
...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/" {
			...
			w.Write([]byte("Service is running perfectly!"))
			return
		}
		...
		if cleanPath == subPath {
			...
			http.Error(w, "Subscription not ready yet", http.StatusNotFound)
			return
		}
	...
	fmt.Printf("Go Native Server running on port %s...\n", port)
